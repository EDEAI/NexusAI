...
class ChatroomAgentRelation(MySQL):
    ...
    def show_chatroom_agent(self, chatroom_id: int = 0): 
        ...
        list = self.select(
            columns=["agents.id AS agent_id", "agents.app_id", "agents.user_id", "apps.name", "apps.description", "apps.icon", "apps.icon_background", "agents.obligations", "chatroom_agent_relation.active"],
            joins=[
