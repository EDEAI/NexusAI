    ...
    def get_app_list(self, page, page_size, search_type, apps_name, apps_mode, user_id, team_id, tag_ids):
        ...
        for info_item in info_workflow:
            if info_item['app_id'] in other_dict:
                ...
                if info_item.get('avatar'):
                    if not info_item['avatar'].startswith(('http://', 'https://')):
                        info_item['avatar'] = f"{settings.STORAGE_URL}/upload/{info_item['avatar']}"
        ...
        for app_item in info_app:
            if app_item['workflow_app_id'] in workflow_dict:
                ...
                if app_item.get('avatar'):
                    if not app_item['avatar'].startswith(('http://', 'https://')):
                        app_item['avatar'] = f"{settings.STORAGE_URL}/upload/{app_item['avatar']}"
        ...
        for data_item in all_app:
            if data_item['avatar']:
                ...
                if not data_item['avatar'].startswith(('http://', 'https://')):
                    data_item['avatar'] = f"{settings.STORAGE_URL}/upload/{data_item['avatar']}"
            if data_item['mode'] == 2:
