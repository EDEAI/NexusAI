...
const useStore = create(
    devtools<AppState>((set, get) => ({
                ...
                if (edge.is_logical_branch) {
                    if (edge.sourceType == BlockEnum.ConditionBranch) {
                        ...
                        if (edge.sourceHandle == 'start_source_else') {
                            ...
                            const id =
                                sourceFreeNode?.data?.logic_branches?.branches[edge.sourceHandle]
                                    ?.id;
                            console.log(111, id);
        ...
        setSelect: (id, update = false) => {
            ...
            // set({
            //     selectedNode: get().nodes.find(node => node.id === id),
            // });

            // get().setShowChildNode(null)
        },
