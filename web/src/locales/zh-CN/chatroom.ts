export default {
    // MCP Tool Display - Status
    'app.chatroom.mcptool.status.completed': '已完成',
    'app.chatroom.mcptool.status.running': '运行中',
    'app.chatroom.mcptool.status.waiting': '等待确认',
    'app.chatroom.mcptool.status.waitingFileUpload': '等待文件上传',
    'app.chatroom.mcptool.status.failed': '失败',
    'app.chatroom.mcptool.status.pending': '待处理',
    
    // MCP Tool Display - Types
    'app.chatroom.mcptool.type.skill': '技能',
    'app.chatroom.mcptool.type.workflow': '工作流',
    'app.chatroom.mcptool.type.tool': '工具',
    
    // MCP Tool Display - UI Labels
    'app.chatroom.mcptool.toolCalls': '工具调用',
    'app.chatroom.mcptool.inputVariables': '输入变量',
    'app.chatroom.mcptool.otherParameters': '其他参数',
    'app.chatroom.mcptool.noParameters': '暂无参数',
    'app.chatroom.mcptool.outputs': '输出结果',
    'app.chatroom.mcptool.noOutputs': '暂无输出',
    'app.chatroom.mcptool.files': '文件列表',
  'app.chatroom.mcptool.inputFiles': '输入文件',
    'app.chatroom.mcptool.downloadFile': '下载文件',
    'app.chatroom.mcptool.parameters': '参数',
    'app.chatroom.mcptool.result': '结果',
    'app.chatroom.mcptool.workflowDetails': '工作流详情',
    'app.chatroom.mcptool.handleTodo': '处理待办',
    'app.chatroom.mcptool.error': '错误',
    
    // MCP Tool Display - Workflow Related
    'app.chatroom.mcptool.workflow': '工作流',
    'app.chatroom.mcptool.confirmer': '确认者',
    'app.chatroom.mcptool.status': '状态',
    'app.chatroom.mcptool.appRunId': '应用运行ID',
    'app.chatroom.mcptool.nodeExecId': '节点执行ID',
    'app.chatroom.mcptool.requiresConfirmation': '需要用户确认',
    
    // Waiting Reply Indicator
    'app.chatroom.waitingReply': '等待回复中...',
    'app.chatroom.mcptool.requiredFiles': '补充文件',
    'app.chatroom.mcptool.completeUpload': '完成上传',
    'app.chatroom.mcptool.noFilesRequired': '无需上传文件。',
    'app.chatroom.mcptool.uploading': '上传中...',
    'app.chatroom.mcptool.uploadFile': '上传文件',
    'app.chatroom.mcptool.remove': '移除',

    // Thinking Block
    'app.chatroom.thinking.content': '思考内容',
    'app.chatroom.thinking.inProgress': '正在进行思考……',
};