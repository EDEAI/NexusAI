...
import { Button, Typography, Tag, Descriptions, Timeline, Spin } from 'antd';
import { memo, useEffect, useState } from 'react';
import ResizablePanel from '@/components/Panel/DraggablePanel';

export default memo(({ data, onClose }: { data: { app_id: string; app_run_id: string }; onClose?: () => void }) => {
    const intl = useIntl();
    const [panelWidth, setPanelWidth] = useState(500);

    const { data: logDetail, loading, run } = useRequest(
                    ...
                    {loading ? (
                        ...
                        <>
                            <Descriptions column={1} bordered size="small">
                                <Descriptions.Item label={intl.formatMessage({ id: 'workflow.user' })}>
                                    {logDetail.nickname}
                                </Descriptions.Item>
                            ...
                            {Array.isArray(logDetail?.prompt_data) && logDetail.prompt_data.length > 0 && (
                                <div className="mt-4">
                                    <Typography.Title level={5}>
                                        {intl.formatMessage({ id: 'agent.log.conversation' })}
                                    </Typography.Title>
