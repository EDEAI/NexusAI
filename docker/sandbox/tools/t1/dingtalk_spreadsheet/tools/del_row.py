...
class DelRowTool(Tool):
    def _invoke(
        ...
        row_count = int(tool_parameters.get("row_count", 0))
        if row_count <= 0:
            yield self.create_text_message('row_count must be greater than 0')
            return
        res = client.delete_rows(table_id, sheet_id, start_row, row_count)
