...
class StopTaskTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        # Debug information
        print("[DEBUG] Starting Stop Task operation")
        ...
        try:
            # Make request to stop task
            url = f"https://api.browser-use.com/api/v1/stop-task"
            headers = {
            ...
            print(f"[DEBUG] Stop Task response status code: {response.status_code}")
            print(f"[DEBUG] Stop Task response content: {response.text}")
            
            # Process response
            if response.status_code == 200:
                print(f"[DEBUG] Task stopped successfully")
                
                # Return success message
                yield self.create_json_message({
                    "status": "success",
                    "message": f"Task {task_id} has been successfully stopped. This task cannot be resumed.",
                    "task_id": task_id
                })
            else:
                error_message = f"Failed to stop task with status code: {response.status_code}"
            ...
            error_message = f"Unexpected error during stop task operation: {str(e)}"
            print(f"[DEBUG] {error_message}")
            yield self.create_json_message({
                "status": "error",
                "message": error_message
