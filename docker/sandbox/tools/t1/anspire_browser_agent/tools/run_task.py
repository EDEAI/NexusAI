...
class AnspireBrowserAgentTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        instance_id = tool_parameters.get("instance_id")
        task_prompt = tool_parameters.get("task_prompt")
        site_url = tool_parameters.get("site_url")
        ...
        if not task_prompt:
            error_message = "Task Prompt description is required"
            print(f"[DEBUG] Error: {error_message}")
            yield self.create_json_message(
                {"status": "error", "message": error_message}
            )
            return
        try:
            _endpoint = "https://plugin.anspire.cn/api/brower/action"
            headers = {
            ...
            if response.status_code == 200:
                try:
                    ...
                    response_data = {
                        "status": "success",
                        "message": f"Task created successfully. Task ID: {task_id} Task Run ID: {result.get('task_run_id')}",
                        "data": result,
                    }
            ...
            error_message = f"Unexpected error run task: {str(e)}"
            print(f"[DEBUG] {error_message}")
            yield self.create_json_message(
                {"status": "error", "message": error_message}
            )
