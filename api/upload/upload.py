...
async def upload_file(file: UploadFile = File(...), chat_base_url: str = None, userinfo: TokenData = Depends(get_current_user)) -> UploadFileResponse:
    ...
    constraint
        Support TXT, MARKDOWN, PDF, HTML, XLSX, PPTX, DOCX, CSV, MP3, OGG, M4A, FLAC, WAV, PNG, JPG, JPEG, each file does not exceed 15MB

    Returns:
        The ID of the newly created file.
    """
    extensions_all = {'.txt', '.md', '.pdf', '.html', '.xlsx', '.pptx', '.docx', '.csv', '.mp3', '.ogg', '.m4a', '.flac', '.wav', '.png', '.jpg', '.jpeg'}
