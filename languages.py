...
language_packs = {
    "en": {
        ...
        "agent_system_prompt_with_auto_match_ability": '''
            ...
            4. Based on the dialogue scenario analysis from point 3, if the dialogue scenario is related to your identity definition, refer to your identity definition. If there is no correlation, completely discard your identity definition and try to adapt to the dialogue scenario to reply;
            5. Through the analysis required in point 4, if you need to refer to the identity definition and I have provided relevant content retrieved from the knowledge base, you must also refer to the relevant content retrieved from the knowledge base when responding;
            6. Based on the current task planning, task progress, and the related tools I have provided, select the most suitable tool to call. You can only call at most one tool at a time. If the tool has been successfully executed and the tool execution result has met the user's current needs, or if you have failed to execute the tool after 3 consecutive attempts, then **DO NOT** call the tool again;
