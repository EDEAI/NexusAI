    ...
    "en": {
        ...
        "agent_system_prompt_with_auto_match_ability": '''
            ...
            ********************End of identity definition content********************
            ...
            5. Based on the current task planning, task progress, and the related tools I have provided, select the most suitable tool for the next step of task processing. Tools can be called repeatedly, with only one tool called at a time. Patiently repeat the above steps until the task processing is completed;
            6. Pay attention to the tool list I provide. Tools with the name prefix "nexusai__workflow" are workflow tools. Some nodes in each workflow may require manual confirmation for their output data. This type of workflow tool has a characteristic feature: the tool input parameters will contain a parameter called "node_confirm_users". Each parameter within this parameter represents a node that requires manual confirmation. You need to select one confirmer from the member list I provide for each node. You should analyze and select the most suitable member as the confirmer based on the current task, node information, and member information. Note that the actual parameter requires the member ID.
