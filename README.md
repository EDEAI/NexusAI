...
[Clone the NexusAI source code to your local environment](https://github.com/EDEAI/NexusAI/wiki/_new#clone-nexusai)

## Server deployment
[Initialize the model directory and storage directory](https://github.com/EDEAI/NexusAI/wiki/_new#initialize-the-model-directory-and-storage-directory)

[Cloning the Embedding and Reranker models for offline mode](https://github.com/EDEAI/NexusAI/wiki/_new#cloning-the-embedding-and-reranker-models-for-offline-mode)
...
Copy `.env` and modify the `.env` configuration items as needed, For configuration instructions, refer to [docker-compose.yml configuration instructions](https://github.com/EDEAI/NexusAI/wiki/_new#docker-composeyml-configuration-instructions)
```bash
cp .env.template .env
```
...
docker run -d -p <SANDBOX_PORT>:8001 -v NexusAI/storage:/storage edeai/sandbox:<tag>
```
...
## WEB deployment
Go to the web directory and copy `envConfig.ts`, Modify the `envConfig.ts` configuration items as needed. For configuration instructions, refer to [docker-compose.yml configuration instructions](https://github.com/EDEAI/NexusAI/wiki/_new#docker-composeyml-configuration-instructions)
```bash
cd web
...
# SANDBOX_PORT should be consistent with .env, and the mounted local storage directory should be changed to the real path
docker run -d -p <SANDBOX_PORT>:8001 -v NexusAI/storage:/storage edeai/sandbox:<new tag>
```
